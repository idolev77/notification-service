...
**Concurrency safety:** the dispatcher's 3-transaction layout (claim → external call → persist outcome) is hardened against duplicate task delivery (broker-redelivered messages, crashed-and-resurrected workers, manual re-enqueues) by a row-level lock acquired in TX1 with `SELECT … FOR UPDATE SKIP LOCKED` (see `_claim_delivery_for_attempt` in [app/services/dispatcher.py](app/services/dispatcher.py)). Three terminal branches:

  1. *Row locked by another worker* — `SKIP LOCKED` returns no row; the dispatcher raises `DeliveryAlreadyClaimedError`, the Celery task acks the duplicate, and the holder finishes the work. Single-attempt guarantee preserved without Postgres advisory locks or external coordination.
  2. *Row already terminal* (`DELIVERED` / `PERMANENTLY_FAILED` / `CANCELLED`) — duplicate Celery message that arrived after the holder wrote the final outcome; the dispatcher raises `DeliveryAlreadyTerminalError` and the task acks silently.
  3. *Row in `SENDING`* — a previous worker committed TX1 (the `QUEUED → SENDING` flip is durable by design) and then died before TX2. This new attempt is treated as fresh: `attempts++`, `last_attempt_at = now`, then proceed to the provider. If `attempts` already equals `max_retry_attempts`, the dispatcher writes `PERMANENTLY_FAILED` in the same TX and exits without contacting the provider — preventing an unbounded recovery loop on a deterministically-failing payload.

**Cancel-vs-scheduler race:** the same `with_for_update()` pattern is mirrored on the cancel path in [app/api/notifications.py](app/api/notifications.py). The scheduler claims due notifications with `SELECT … FOR UPDATE SKIP LOCKED`; cancel takes a plain `FOR UPDATE` on the single target row. Whichever transaction grabs the row first wins; the loser sees the committed `PROCESSING` (or `CANCELLED`) status under the lock and refuses the now-illegal transition with HTTP 409, eliminating the "cancelled-but-already-dispatched" lie.

**Drop visibility on infrastructure failure:** the `POST /notifications` endpoint distinguishes two drop categories. *Legitimate filters* (paused user, quiet hours, frequency cap tripped, no enabled channels, no resolvable address, missing preferences) return `202 Accepted` with explicit `is_dropped=true` + `drop_reason` + `drop_detail` fields on the response — the request was valid, the system correctly chose not to send, and the caller has structured visibility. *Infrastructure failures* (currently: Redis cap-evaluator unreachable while `rate_limiter_fail_open=False`) raise `InfrastructureUnavailableError` in the service layer, which the API translates into `503 Service Unavailable` with a `Retry-After: 5` header and rolls back the transaction — nothing is persisted to mislead the audit trail. Without this split, a security-critical send (password reset, fraud alert) could silently disappear into a `COMPLETED` row whenever Redis blipped.
